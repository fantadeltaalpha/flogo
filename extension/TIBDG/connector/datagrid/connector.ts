...
@WiContrib({})
@Injectable()
export class TibcoDatagridConnectorContribution extends WiServiceHandlerContribution {
  constructor() {
    super();
  }
  ...
  value = (
    fieldName: string,
    context: IConnectorContribution
  ): Observable<any> | any => {
    return null;
  };
  ...
  validate = (
    ...
    if (name === "Save") {
      let url: IFieldDefinition = context.getField("realmUrl");
      let gridName: IFieldDefinition = context.getField("gridName");
      if (
        url.value &&
        url.value !== "" &&
        gridName.value &&
